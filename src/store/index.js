...
import {loginApi, userInfoApi, mockServer} from '@/const/api'
...
export const mutations = {
  login(state, payload) {
    ...
    cookie.set('token', payload.token, {path})
...
export const actions = {
  async login(context, payload) {
    ...
    let resp = await this.$axios.$post(loginApi, payload)
  ...
  async fetchMetaInfo({commit}, {projectNo}) {
    // let resp = await this.$axios.$get(`${mockServer}/security/api/v1/configs`)
    // let meta = {...resp.payload}
