<template>
  <div class="group-edit">
    <el-form-renderer
      ...
      :disabed="$route.query.isView"
    >
      <el-form-item slot="id:name" label="拼团类型" style="width:100%;">
        <el-radio-group v-model="formData.groupType">
          <el-radio :label="'1'">普通拼团</el-radio>
          <el-radio :label="'2'">老带新拼团</el-radio>
        </el-radio-group>
        <span style="color:'#ccc';line-height:36px;margin-left:10px;"
          >（所有会员可发起拼团，仅新会员可参与拼团）</span
        >
      </el-form-item>
      <el-form-item slot="id:stockCount" label="选择商品" style="width:100%;">
          ...
          >
            ...
            <el-table-column width="100">
              <template slot-scope="scope">
                <img
                  :src="
                    scope.row.imageUrl ? scope.row.imageUrl.split(',')[0] : ''
                  "
                  width="50"
                  height="50"
                />
      ...
      <el-form-item
        ...
        :rules="[
          {
            validator: groupPirceValid.call(this),
            trigger: 'blur'
          }
        ]"
      >
        ...
        <el-row type="flex" justify="start" :gutter="24" style="padding:10px;">
          <el-checkbox v-model="formData.isGroup"
            >开启团主优惠价，开团价格</el-checkbox
          >
          <el-input
      ...
      <!-- 默认插槽 -->
      <template name="other-item">
        <el-form-item label="商品限购" style="width:100%;">
          <el-checkbox v-model="formData.checked1"
            >开启限购，每人最多购买</el-checkbox
          >
          <el-inpu-number
            style="width:25%;"
            v-model="formData.astrictNUm"
            :disabed="!formData.checked1"
          ></el-inpu-number>
        </el-form-item>
        <el-form-item label="优惠券使用限制" style="width:100%;">
          <el-checkbox v-model="formData.checked2">允许使用优惠券</el-checkbox>
        </el-form-item>
        <el-form-item label="虚拟成团" style="width:100%;">
          <el-checkbox v-model="formData.checked3"
            >开启后，当有人发起拼团或参团，有效期内未达到成团人数也默认自动成团</el-checkbox
          >
        </el-form-item>
      </template>
    </el-form-renderer>
    ...
    >
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="saveSelectCommodity">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
const groupPirceValid = function() {
  return (rule, value, callback) => {
    if (!this.formData.sellPrice && !this.formData.groupPrice) {
      return callback('请至少填写一种拼团价格方式')
    }
    callback()
  }
}
...
export default {
  components: {
    'el-inpu-number': () => import('element-ui/packages/input-number/index')
  },
  data() {
    return {
      // 表单区域 loading
  ...
  created() {
    ...
    this.groupPirceValid = groupPirceValid
  ...
  methods: {
    ...
    saveGroup(isDraft = false) {
      this.$refs.form.validate(valid => {
        ...
        const lastParmas = Object.assign({}, data, this.formData, {
          id: new Date().getTime(),
          orderAmount: parseInt(Math.random() * 5000),
          orderCount: parseInt(Math.random() * 100),
          peopleCount: parseInt(Math.random() * 212),
          status: Math.round(Math.random() * 5)
        })

        const urlOne = this.id ? commonEdit : commonAdd
        const body = !this.id
          ? lastParmas
          : {
              args: {
        ...
        this.$axios
          .$post(urlOne + '?collection=activityDetail', body)
          .then(res => {
            this.$message.success('保存成功')
            this.$router.push('/activity-management/activity-list')
          })
          .catch(err => {
    ...
    async fetchDetail(id) {
      ...
      try {
        const {payload} = await this.$axios.$post(
          commonDetail + '?collection=activityDetail',
          {id}
        )
        this.$refs.form.updateForm(payload)
...
.group-edit {
  ...
  /deep/ .el-input, /deep/ .el-date-editor, /deep/ .el-input-number {
    width: 100%;
  }
